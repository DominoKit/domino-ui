...
import static org.dominokit.domino.ui.menu.direction.DropDirection.BEST_MIDDLE_UP_DOWN;

public class DateBox extends TextInputFormField<DateBox, HTMLInputElement, Date> implements HasLabels<CalendarLabels>, CalendarViewListener {
    ...
    public DateBox(String label, Date date, DateTimeFormatInfo dateTimeFormatInfo, CalendarInitConfig calendarInitConfig) {
        this(date, dateTimeFormatInfo, calendarInitConfig);
        setLabel(label);
    }

    public DateBox(Date date, DateTimeFormatInfo dateTimeFormatInfo, CalendarInitConfig calendarInitConfig) {
        this.value = date;
        this.calendar = Calendar.create(date, dateTimeFormatInfo, calendarInitConfig);
        this.pattern = dateTimeFormatInfo.dateFormatFull();
        this.popover = Popover.create(this.getWrapperElement())
                .setOpenOnClick(this.openOnClick)
                .setPosition(BEST_MIDDLE_UP_DOWN)
                .appendChild(this.calendar)
                .addOnRemoveListener(popover -> {
                    withOpenOnFocusToggleListeners(false, field -> focus());
                });
        onDetached(mutationRecord -> popover.close());

        getInputElement()
                .onKeyPress(keyEvents -> keyEvents
                        .onEnter(evt -> doOpen())
                        .onSpace(evt -> doOpen())
                );
        ...
        appendChild(PrimaryAddOn.of(getConfig().defaultDateBoxIcon()
                .get()
                .clickable()
    ...
    protected void doSetValue(Date value) {
        ...
        withDateSelectionToggleListeners(false, field -> {
            this.calendar.setDate(this.value);
        });
    }

    private void updateStringValue() {
        getInputElement().element().value = getFormatted(this.value, this.calendar.getDateTimeFormatInfo());
    }
