...
public class DefaultFilePreview extends BaseDominoElement<HTMLDivElement, DefaultFilePreview>
    ...
    private static final CompositeCssClass successBorder = CompositeCssClass.of(dui_border, dui_border_solid, dui_border_success);
    private static final CompositeCssClass failedBorder = CompositeCssClass.of(dui_border, dui_border_solid, dui_border_error);
    private static final CompositeCssClass canceledBorder = CompositeCssClass.of(dui_border, dui_border_solid, dui_border_warning);
    ...
    public DefaultFilePreview(FileItem fileItem) {
        ...
        progress = Progress.create()
                .addCss(dui_h_2)
                .appendChild(progressBar = ProgressBar.create(this.fileItem.getFile().size).setValue(20));
        ...
        if (this.fileItem.isImage()) {
            ...
            fileReader.addEventListener("load", evt -> {
                thumbnail
                        .appendChild(img(fileReader.result.asString())
        ...
        thumbnail.withFooter((parent, footer) -> {
            footer
                    .appendChild(BlockHeader.create(this.fileItem.getFile().name, this.fileItem.readableFileSize())
                            .addCss(dui_text_center)
                            .withHeaderElement((blockHeader, header) -> header.addCss(dui_text_ellipsis))
                            .withDescriptionElement((blockHeader, description) -> description.addCss(dui_text_ellipsis))
                            .setTooltip(this.fileItem.getFile().name + ": " + this.fileItem.readableFileSize())
                    )
                    ...
                    .appendChild(div()
                            ...
                            .appendChild(uploadIcon
                                    .addCss(dui_fg_accent)
                                    .clickable()
                                    .addClickListener(evt -> {
                                        messageElement
                                                .clearElement()
                                                .removeCss(statusMessageCss);
                                        fileItem.upload();
                                    })
                            )
                            .appendChild(cancelIcon
    ...
    public void onUploadFailed(String error) {
        ...
        messageElement
                .addCss(failedCss).setTextContent(error)
                .setTooltip(error);
        cancelIcon.hide();
        uploadIcon.show();
        removeIcon.show();
    ...
    public void onUploadSuccess() {
        ...
        messageElement
                .addCss(successCss)
                .setTextContent(getLabels().getDefaultUploadSuccessMessage());
        cancelIcon.hide();
        uploadIcon.show();
        removeIcon.show();
