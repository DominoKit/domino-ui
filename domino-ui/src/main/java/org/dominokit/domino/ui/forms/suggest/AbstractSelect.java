...
public abstract class AbstractSelect<
        T, V, E extends IsElement<?>, O extends Option<T, E, O>, C extends AbstractSelect<T, V, E, O, C>>
    ...
    public AbstractSelect() {
        ...
        optionsMenu =
                Menu.<T>create()
                        ...
                        .addSelectionListener((source, selection) -> source.flatMap(OptionMeta::<T, E, O>get).ifPresent(meta -> onOptionSelected(meta.getOption())))
        ...
        getInputElement().onKeyDown(keyEvents -> keyEvents
                .onEnter(evt -> openOptionMenu())
                .onSpace(evt -> openOptionMenu())
        );
    ...
    protected C clearValue(boolean silent) {
        if (!optionsMenu.getSelection().isEmpty()) {
            ...
            optionsMenu.withPauseSelectionListenersToggle(
                    true,
                    field -> optionsMenu.getSelection().forEach(AbstractMenuItem::deselect));
    ...
    public C group(Menu.MenuItemsGroupHandler<T, AbstractMenuItem<T>> groupHandler, Collection<O> options) {
    ...
    public C selectOption(O option) {
        findOption(option).ifPresent(menuItem -> menuItem.getMenuItem().select(isChangeListenersPaused()));
        return (C) this;
    }
                ...
                .flatMap(
                        vSelectionOption -> {
                            return optionsMenu.getFlatMenuItems().stream()
                                    ...
                                    .map(menuItem -> OptionMeta.<T, E, O>get(menuItem)
                                            .get()
                                            .getOption()
                                    )
                                    .findFirst();
    ...
    public Optional<O> findOptionByIndex(int index) {
        if (index < optionsMenu.getFlatMenuItems().size() && index >= 0) {
            ...
            return Optional.ofNullable(OptionMeta.<T, E, O>get(menuItem)
                    .get()
                    .getOption());
        }
    ...
    public C removeOption(O option) {
        findOption(option).ifPresent(found -> {
            option.remove();
    ...
    public C removeAllOptions() {
        new ArrayList<>(optionsMenu.getMenuItems())
                .forEach(menuItem -> {
                    removeOption(OptionMeta.<T, E, O>get(menuItem)
                            .get()
                            .getOption()
                    );
                });
    ...
    private static class SelectAutoValidator<
            T, V, E extends IsElement<?>, S extends Option<T, E, S>, C extends AbstractSelect<T, V, E, S, C>>
