...
public class Calendar extends BaseDominoElement<HTMLDivElement, Calendar> implements CalendarStyles, IsCalendar, HasChangeListeners<Calendar, Date> {
    ...
    public Calendar(Date date, DateTimeFormatInfo dateTimeFormatInfo, CalendarInitConfig config
        ...
        this.root = div()
                ...
                .appendChild(calendarBody = div()
                        .addCss(dui_calendar_body)
                        .appendChild(calendarMonth = CalendarMonth.create(this))
                        .appendChild(yearMonthPicker = YearMonthPicker.create(this)
                                .hide()
                        )
                )
                .apply(calendar -> this.footer = LazyChild.of(div().addCss(dui_calendar_footer), calendar));

        this.root.addEventListener(CalendarCustomEvents.DATE_NAVIGATION_CHANGED, evt -> {
            evt.stopPropagation();
            CalendarCustomEvents.UpdateDateEventData dateData = CalendarCustomEvents.UpdateDateEventData.of((CustomEvent<?>) evt);
        ...
        this.root.addEventListener(CalendarCustomEvents.DUI_CALENDAR_DATE_SELECTION_CHANGED, evt -> {
            evt.stopPropagation();
            CalendarCustomEvents.UpdateDateEventData dateData = CalendarCustomEvents.UpdateDateEventData.of((CustomEvent<?>) evt);
        ...
        this.root.addEventListener(CalendarCustomEvents.SELECT_YEAR_MONTH, evt -> {
            evt.stopPropagation();
    ...
    public static Calendar create(Date date, DateTimeFormatInfo dateTimeFormatInfo, CalendarInitConfig config) {
    ...
    public Calendar triggerChangeListeners(Date oldValue, Date newValue) {
        if(!this.changeListenersPaused){
            this.changeListeners.forEach(changeListener -> changeListener.onValueChanged(oldValue, newValue));
        }
        return this;
    }
