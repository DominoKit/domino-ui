...
public class GroupingPlugin<T> implements DataTablePlugin<T>, TableConfig.RowAppender<T> {
     ...
     */
    public GroupingPlugin<T> setGroupExpandedIcon(Supplier<ToggleIcon<?,?>> groupExpandedIconSupplier) {
        this.groupExpandCollapseIconSupplier = groupExpandedIconSupplier;
        return this;
    }
    ...
    public void appendRow(DataTable<T> dataTable, TableRow<T> tableRow) {
        ...
        if (!dataGroups.containsKey(groupId)) {
            HTMLTableCellElement cellElement =
                    td().setAttribute("colspan", dataTable.getTableConfig().getColumns().size() + "").element();
            ...
            ToggleIcon<?,?> groupIconSupplier =
                    groupExpandCollapseIconSupplier
                            .get()
                            .clickable()
            ...
            dataTable.bodyElement().appendChild(tr().css("data-table-group-row").appendChild(cellElement));
            ...
            if (nonNull(nextSibling)) {
                dataTable.bodyElement().insertBefore(tableRow.element(), nextSibling);
            } else {
                dataTable.bodyElement().appendChild(tableRow.element());
            }
    ...
    public static class DataGroup<T> implements ComponentMeta {
        ...
        public void toggleGroup() {
            expanded = !expanded;
            groupRows.forEach(tableRow -> TableRowElement.of(tableRow.element()).toggleDisplay(expanded));
        }
        ...
        public void render() {
            elements.elementOf(cellInfo.getElement())
                    .clearElement()
                    .appendChild(
                            FlexLayout.create()
