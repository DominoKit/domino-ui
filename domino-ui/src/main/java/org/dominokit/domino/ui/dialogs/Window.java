...
public class Window extends AbstractDialog<Window> {
    ...
    onResize(
        (element1, observer, entries) -> updatePosition());
        ...
        if (maximized) {
            ...
            if (windowLeft > 0 ){
                if(windowLeft < (windowWidth - initialWidth)) {
                    ...
                    modalElement.element().style.left = windowLeft - ((windowLeft +initialWidth) - windowWidth - DomGlobal.window.pageXOffset) + "px";
                }
            } else {
            ...
            if (windowTop > 0 ){
                if(windowTop < (windowHeight - initialHeight)) {
                    ...
                    modalElement.element().style.left = windowTop - ((windowLeft + initialHeight) - windowHeight - DomGlobal.window.pageYOffset ) + "px";
                }
            } else {
                modalElement.element().style.top =(fixed ? 0 : DomGlobal.window.pageYOffset) - ((initialHeight - windowHeight) / 2) + "px";;
            }
    ...
    private void removeMoveListeners() {
        ...
        headerElement.element()
                .removeEventListener(EventType.mouseup.getName(), stopMoveListener); headerElement.element()
                .removeEventListener(EventType.touchend.getName(), stopMoveListener);
    }

    private void addMoveListeners() {
        headerElement.get()
                .addEventsListener(evt -> {
                    if (draggable) {
                        MouseEvent mouseEvent = Js.uncheckedCast(evt);
