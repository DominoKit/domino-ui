...
public class Menu<V> extends BaseDominoElement<HTMLDivElement, Menu<V>>
    implements HasSelectionListeners<
        Menu<V>, AbstractMenuItem<V, ?>, List<AbstractMenuItem<V, ?>>> {
  ...
  public Menu<V> setTargetElement(HTMLElement targetElement) {
    ...
    if (nonNull(this.targetElement)) {
      ...
      setDropDown(true);
      DominoElement.of(this.targetElement)
          .onDetached(mutationRecord -> document.removeEventListener("scroll", repositionListener));
    } else {
      setDropDown(false);
    }
    return this;
