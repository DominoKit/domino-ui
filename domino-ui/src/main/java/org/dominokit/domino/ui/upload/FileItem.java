...
public class FileItem extends BaseDominoElement<HTMLElement, FileItem> {
     ...
     * @return new instance
     */
    public static FileItem create(File file, UploadOptions options, FilePreviewFactory previewFactory) {
    ...
    public void upload(UploadRequestSender requestSender) {
        ...
        if (!isExceedsMaxFile() && !uploaded && !isCanceled()) {
            ...
            request.upload.addEventListener(
                    "loadstart",
                    evt -> filePreview.onUploadStarted());

            request.upload.addEventListener(
                    "loadend",
                    evt -> filePreview.onUploadCompleted());
    ...
    public void validateSize() {
        if(isExceedsMaxFile()) {
            filePreview.onUploadFailed("File is too large, maximum file size is " +
                    formatSize(options.getMaxFileSize()));
        }
    }
