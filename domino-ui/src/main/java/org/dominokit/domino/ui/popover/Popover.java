 ...
 * <p>
 * Customize the component can be done by overwriting classes provided by {@link PopoverStyles}
 *
 * <p>For example: </p>
 * <pre>
...
public class Popover extends BaseDominoElement<HTMLDivElement, Popover> implements Switchable<Popover> {
    ...
    private final DominoElement<HTMLHeadingElement> headingElement = DominoElement.of(h(3).css(POPOVER_TITLE));
    private final DominoElement<HTMLDivElement> contentElement = DominoElement.of(div().css(POPOVER_CONTENT));
    ...
    public Popover(HTMLElement target, String title, Node content) {
        ...
        init(this);
        onDetached(mutationRecord -> document.body.removeEventListener(EventType.keydown.getName(), closeListener));
    }
    ...
    /**
     * Closes the popover and remove it completely from the target element so it will not be shown again
     */
    public void discard() {
        close();
    ...
    /**
     * Creates new instance hidden and with no paddings by default; this is helpful for pickers inside {@link TimeBox} and {@link DateBox}
     *
