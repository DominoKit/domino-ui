...
public class TreeHeightCollapseStrategy implements CollapseStrategy, CollapsibleStyles {
    ...
    private double getActualHeight() {
        double expandedHeight = treeItem.childElements()
                .stream()
    ...
    private void expandElement(Element element) {
        if (dui_transition_none.isAppliedTo(treeItem)) {
            ...
            treeItem.element().addEventListener("webkitTransitionEnd", stopListener, addEventListenerOptions);
                ...
                .ifPresent(
                        parent -> {
                            if (parent instanceof TreeItem) {
                                ...
                                parentItem
                                        .removeCssProperty(parentItem.getAttribute(DUI_EXPAND_COLLAPSE_VAR));
