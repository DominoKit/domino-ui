...
public class FileUpload extends BaseDominoElement<HTMLDivElement, FileUpload> implements HasName<FileUpload> {
    ...
    private final DominoElement<HTMLDivElement> formElement = DominoElement.of(div().css("file-upload"));
    private final DominoElement<HTMLDivElement> uploadMessageContainer = DominoElement.of(div().css("file-upload-message"));
    private final DominoElement<HTMLDivElement> uploadIconContainer = DominoElement.of(div().css("file-upload-message-icon"));
    ...
    public FileUpload() {
        ...
        formElement.addEventListener("dragover", evt -> {
            dropEffect.ifPresent(effect -> ((DragEvent) evt).dataTransfer.dropEffect = effect.getEffect());
            addHover();
            evt.stopPropagation();
            evt.preventDefault();
        });
    ...
    /**
     * Sets that the file should be uploaded only when calling {@link FileUpload#uploadFiles(FileList)} or {@link FileUpload#uploadAllFiles()}
     *
     * @return same instance
     */
